        ...
        public async Task<object> Execute(Recommendation recommendation, OptionSettingItem optionSetting)
        {
            ...
            if (createNewVPCConnector)
            {
                ...
                var userInputConfigurationVPCs = new UserInputConfiguration<Vpc>(
                    vpc => vpc.VpcId,
                    vpc => vpc.VpcId,
                    vpc => !string.IsNullOrEmpty(currentVpcValue) ? vpc.VpcId.Equals(currentVpcValue) : vpc.IsDefault)
                {
                    CanBeEmpty = false,
                    CreateNew = false
                };
                ...
                var userInputConfigurationSubnets = new UserInputConfiguration<Subnet>(
                    ...
                    subnet => $"{subnet.SubnetId.PadRight(24)} | {subnet.VpcId.PadRight(21)} | {subnet.AvailabilityZone}",
                    subnet => false)
                {
                    CanBeEmpty = true,
                    CreateNew = false
                ...
                var userInputConfigurationSecurityGroups = new UserInputConfiguration<SecurityGroup>(
                    ...
                    securityGroup => $"{securityGroup.GroupName.PadRight(groupNamePadding)} | {securityGroup.GroupId.PadRight(20)} | {securityGroup.VpcId}",
                    securityGroup => false)
                {
                    CanBeEmpty = true,
                    CreateNew = false
